...
1.  Use the `Room Temp Source` selection box (`SETTINGS Z1 Section`) to select the room temperature source. `Choose Asgard virtual thermostat`
2.  Use the `Temp Sensor Source` selection box ((`VIRTUAL THERMOSTAT Z1 Section`)) to select the temperature sensor source for the virtual thermostat. 
...
> [!TIP]
> **Greyed thermostats:** When `SW1-8` is in the `OFF` position, the Room thermostats will be disabled. When `SW1-8` is in the `ON` position, the Virtual thermostats will be disabled. 
