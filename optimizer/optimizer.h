...
inline void run_auto_adaptive_loop() {
...
inline void predictive_short_cycle_check() {
...
inline void check_lockout_expiration() {
...
inline void on_compressor_stop() {
...
inline void on_compressor_state_change(bool x, bool x_previous) {
  if (x_previous && !x) { // ON -> OFF
    ESP_LOGI(esphome::ecodan::TAG, "Compressor STOP detected");
    on_compressor_stop(); 
  } 
  else if (!x_previous && x) { // OFF -> ON
    auto& status = id(ecodan_instance).get_status();
    if (!status.DefrostActive) {
