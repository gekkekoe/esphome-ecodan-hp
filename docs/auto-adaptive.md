...
| **`Auto-Adaptive: Thermostat Overshoot Compensation`** | Tells the algorithm to ignore a certain amount of overshoot caused by an external thermostat's own hysteresis. | **Default**: `0.0°C`<br>Set this to the known overshoot of your thermostat (e.g., `0.3°C`) to prevent incorrect calculations. |
