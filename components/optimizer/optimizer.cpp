    ...
    namespace optimizer
    {
            ...
            float &out_flow_cool)
        {
            ...
            else if (is_cooling_mode && is_cooling_active)
            {
                ...
                if (calculated_flow > this->state_.cooling_smart_start_temp->state)
                {
                    calculated_flow = this->state_.cooling_smart_start_temp->state;
                    ESP_LOGD("auto_adaptive", "Z%d COOLING: Using Smart Start Temp of %.1f°C", (i + 1), calculated_flow);
                }
                if (calculated_flow < this->state_.minimum_cooling_flow_temp->state)
                {
        ...
        void Optimizer::run_auto_adaptive_loop()
        {
            ...
            if (status.Operation == esphome::ecodan::Status::OperationMode::DHW_ON || status.Operation == esphome::ecodan::Status::OperationMode::FROST_PROTECT || status.Operation == esphome::ecodan::Status::OperationMode::LEGIONELLA_PREVENTION)
            {
                ESP_LOGD("auto_adaptive", "No climate task. Exiting.");
                return;
            }

            if (status.DefrostActive || this->state_.status_short_cycle_lockout->state)
            {
            ...
            {
                ESP_LOGW("auto_adaptive", "Zone 1 Min/Max conflict. Forcing Min (%.1f) to Max (%.1f)", min_flow_z1, max_flow_z1);
            ...
            if (status.has_2zones())
            {
                ...
                {
                    ESP_LOGW("auto_adaptive", "Zone 2 Min/Max conflict. Forcing Min (%.1f) to Max (%.1f)", min_flow_z2, max_flow_z2);
        ...
        void Optimizer::predictive_short_cycle_check()
        {
            ...
            if (status.DefrostActive || this->state_.status_short_cycle_lockout->state || !this->state_.status_compressor->state)
            {
        ...
        void Optimizer::start_lockout()
        {
            ...
            if (duration_s > 0)
            {
                *(this->state_.lockout_expiration_timestamp) = (uint32_t)(current_timestamp + duration_s);
                ESP_LOGI("short_cycle_mitigation", "Lockout active. Expiration timestamp set to: %u", *(this->state_.lockout_expiration_timestamp));
            }
        }
